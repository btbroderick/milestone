...
getFreqVals<-function(dat,nE,MM=1000,totAcc=2*nE,shouldJT=1){
  cTime<-max(dat)
...
getFreqInts<-function(dat,nE,MM=1000,totAcc=2*nE,alpha=.05,shouldJT=1){
  cTime<-max(dat)
  wP<-lik(dat,cTime,"weibull")[c(2,1)]
  lnP<-lik(dat,cTime,"lnorm");
