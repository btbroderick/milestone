...
tempdat <- read.csv(here::here("exampledata","sample.csv"))
...
plotdata <- data.frame(method = methodText,
  ...
  mutate(type = case_when(
    ...
    color = case_when(
      type == "Bayesian" & label == "Predictive Synthesis (MSPE)" ~ 1,
      TRUE ~ 0
    ))
...
p <- ggplot(plotdata, aes(x = method, y = mean, xmin = lower, xmax = upper)) + 
  geom_pointrangeh() +
